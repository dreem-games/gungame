...
public class BodyEditorLoader {
    ...
    public void attachFixture(Body body, String name, FixtureDef fd, Vector2 scale) {
        ...
        Vector2 origin = setAndMultiply(vec, rbModel.origin, scale);
        ...
        for (int i=0, n=rbModel.polygons.size(); i<n; i++) {
            ...
            for (int ii=0, nn=vertices.length; ii<nn; ii++) {
                vertices[ii] = setAndMultiply(newVec(), polygon.vertices.get(ii), scale);
        ...
        for (int i=0, n=rbModel.circles.size(); i<n; i++) {
            ...
            Vector2 center = setAndMultiply(newVec(), circle.center, scale);
            float radius = circle.radius * Math.min(scale.x, scale.y);
    ...
    private Vector2 setAndMultiply(Vector2 origin, Vector2 value, Vector2 scale) {
        return origin.set(value.x * scale.x, value.y * scale.y);
    }
    ...
    public Vector2 getOrigin(String name, Vector2 scale) {
        ...
        return setAndMultiply(vec, rbModel.origin, scale);
    }
